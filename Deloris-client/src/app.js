...
var Scene = (function () {
    ...
    Scene.prototype.createScene = function () {
        ...
        for (var y = size; y <= this.isoArcade.bounds.frontY - size; y += size) {
            for (var x = size; x <= this.isoArcade.bounds.frontX - size; x += size) {
                tile = this.iso.addIsoSprite(x, y, 0, 'tileset', tileArray[tiles[i]], this.isoGroup);
        ...
        window.setInterval(function () {
            var curX = Math.floor(_this.players[_this.playerIndex].body.x / 32);
            var curY = Math.floor(_this.players[_this.playerIndex].body.y / 32);
            var needSend = false;
            if (prevPosX !== curX) {
                needSend = true;
                prevPosX = curX;
            ...
            if (needSend)
                _this.network.sendMove(_this.playerIndex, curX, curY);
        }, 1000);
    };
    ...
    Scene.prototype.addPlayers = function (players) {
        ...
        players.forEach(function (player) {
            var tile = _this.iso.addIsoSprite(player.x * 32, player.y * 32, 0, 'tileset', "mushroom", _this.isoGroup);
            tile.anchor.set(0.5, 1);
            tile.body.collideWorldBounds = true;
            _this.players.push(tile);
        });
    };
